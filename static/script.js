...
function renderHistory(){
  ...
  conversations.forEach((chat,index)=>{
    ...
    deleteBtn.onclick=e=>{
      e.stopPropagation(); conversations.splice(index,1);
      renderHistory(); if(currentChat===chat) currentChat=null; renderChat();
    };
    item.appendChild(deleteBtn);

    item.onclick=()=>{currentChat=conversations[index]; renderChat();}
...
async function sendMessage(){
  ...
  }catch(err){typingDiv.textContent="Error fetching response";}
}
...
// ------------------ Logout ------------------
function logoutUser(){fetch('/logout').then(()=>location.reload()).catch(console.error);}
